...
PRODUCT_IDS = {
    "kasd": "2022070510803319006",
    "mkcs": "2022110410804655577",
    "lkso": "2025041010812405682",
    "njd": "2023062710806629654",
    "njk": "2022061810802884853",
...
async def start_checkout(update: Update, context: CallbackContext):
    # Mengecek apakah bot admin di grup
...
async def product_id_input(update: Update, context: CallbackContext):
    ...
    if product_name not in PRODUCT_IDS:
        await update.message.reply_text(f"masukan tes!")
        return PRODUCT_ID_INPUT
    ...
    await update.message.reply_text(f"Product {product_name} telah ditambahkan. Sekarang, masukkan kunci untuk produk ini (pisahkan dengan baris baru):")
    return COOKIE_INPUT
...
async def cookie_input(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text("ketik n")
...
async def finalize_checkout(update: Update, context: CallbackContext):
    if update.message.text.lower() == "n":
        ...
        await update.message.reply_text(f"Proses checkout selesai! cek pesanan!")
        return ConversationHandler.END
    else:
